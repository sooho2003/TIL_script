...
loginBtn.addEventListener('click',()=>{
    if(userId.value == ''&& userPw.value == ''){
        ...
        //아이디와 비밀번호가 DB서버의 0인덱스값과 모두 일치할 때 네이버 메이페이지 이동
        window.location.href = 'https:..naver.com'
    }else {
        //위 모든 조건이 거짓일때
        errorMsg.textContent = '아이디(로그인 전화번호, 로그인 전용 아이디) 또는 비밀번호가 잘못 되었습니다. 아이디와 비밀번호를 정확이 입력해 주세요';
    }
    //삼항조건 활용 조건1
    //조건식 ? 조건식참결과 : 조건식거짓결과
...
//조건3조건1,2가 모두 거짓일때 아이디를 입력안했는가(참)